...
filters = diffraction_generator.step(
    coeff=torch.zeros(3, n_zernike, device=device, dtype=dtype)
)
plot(
...
physics = SpaceVaryingBlur(
    method="product_convolution2d", **params_pc, device=device, dtype=dtype
)
