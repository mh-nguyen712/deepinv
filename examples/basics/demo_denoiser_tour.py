...
dncnn = dinv.models.DnCNN(device=device)
drunet = dinv.models.DRUNet(device=device)
swinir = dinv.models.SwinIR(device=device)
scunet = dinv.models.SCUNet(device=device)
...
noise_levels = torch.logspace(-2, 0, 9, device=device, dtype=dtype)
noise = torch.randn((len(noise_levels), *image.shape[1:]), device=device, dtype=dtype)
